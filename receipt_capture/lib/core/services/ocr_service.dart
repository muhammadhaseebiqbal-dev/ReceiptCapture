...
class OCRService {
  ...
  static const List<String> knownCompanies = [
    'walmart', 'target', 'amazon', 'starbucks', 'mcdonalds', 'mcdonald\'s',
    'home depot', 'homedepot', 'best buy', 'bestbuy', 'costco', 'cvs', 
    'walgreens', 'kroger', 'publix', 'safeway', 'whole foods', 'wholefoods',
    'trader joe\'s', 'traderjoes', 'shell', 'exxon', 'bp', 'chevron',
    'subway', 'chipotle', 'panera', 'taco bell', 'pizza hut', 'dominos',
    'kfc', 'burger king', 'wendy\'s', 'dunkin', 'tim hortons',
    'apple', 'microsoft', 'google', 'netflix', 'spotify', 'uber', 'lyft'
  ];
  ...
  double _generateRealisticAmountForCompany(String company) {
    ...
    } else if (companyLower.contains('mcdonald') || companyLower.contains('subway') || companyLower.contains('taco bell')) {
      // Fast food: $5-25
      return 5.0 + random.nextDouble() * 20.0;
    } else if (companyLower.contains('walmart') || companyLower.contains('target') || companyLower.contains('costco')) {
      // Retail stores: $15-200
      return 15.0 + random.nextDouble() * 185.0;
    } else if (companyLower.contains('gas') || companyLower.contains('shell') || companyLower.contains('exxon')) {
      // Gas stations: $20-80
      return 20.0 + random.nextDouble() * 60.0;
    } else if (companyLower.contains('grocery') || companyLower.contains('kroger') || companyLower.contains('publix')) {
  ...
  String _generateSimulatedText(String? company, double? amount) {
    ...
    lines.add('Transaction ID: ${random.nextInt(999999).toString().padLeft(6, '0')}');
    lines.add('');
