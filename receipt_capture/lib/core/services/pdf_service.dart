  ...
  Future<String?> convertImageToPdf(String imagePath) async {
    try {
      ...
      if (externalDir != null) {
        ...
        receiptCaptureDir = Directory(path.join(externalDir.path, 'Receipt Capture'));
      } else {
        // Fallback to app documents directory
        final appDocDir = await getApplicationDocumentsDirectory();
        receiptCaptureDir = Directory(path.join(appDocDir.path, 'Receipt Capture'));
      }
  ...
  Future<bool> receiptCaptureFolderExists() async {
    try {
      ...
      if (externalDir != null) {
        ...
        if (androidIndex >= 0) {
          documentsPath = '/${pathSegments.sublist(1, androidIndex).join('/')}/Documents';
        } else {
      ...
      final receiptCaptureDir = Directory(path.join(documentsPath, 'Receipt Capture'));
